...
標準円盤モデルは，重力ポテンシャルが熱に変わり，輻射によって放出することである．

$Q_{\rm rad}^{-} = Q_{\rm vis}^{+} = Q_{\rm grav}^{+}$

重力エネルギーが粘性運動によって熱になり，光度で放出される局所的エネルギー収支過程を表す式．標準モデルは重力を光にするものである．


1. 低光度では失効，広帯域放射が説明できない．
2. 高光度では失効，薄い円盤（光学的に浅い）が必要
3. 低温領域では，水素電離しにくい．The SS disk model breaks down at low temperatures below 10000 K
