...
- ニースモデル
巨大惑星の軌道の力学的進化が太陽系に大きな変動をもたらすモデル．惑星形成後に残った微惑星が巨大惑星の引力で太陽系外側へ散乱される．（補足必要）
...
「衝突脱ガス，分化や化学反応による物質の化学変化」，天体が成分によって成層する．
---
...
（光度：単位時間当たり放射するエネルギー$[\mathrm{erg\ s^{-1}}]$）
...
3. 太陽系元素存在度：ガス量を推測する．太陽と同じ組成と仮定
$$
...
\rho = n_{mol} \mu m_H
$$
ここ，$\mu$は平均分子量，$m_H$は水素の分子量．整理し
$$
P = \frac{\rho k_\mathrm{B} T}{\mu m_\mathrm{H}}
$$
...
音速は（定義式）
$$
 c_\mathrm{s}^2=  \left(\frac{\partial P}{\partial \rho}\right) = \frac{k_BT}{\mu m_H}
$$
...
ガス円盤の基本進化は粘性拡散である．粘性によって角運動量を外側へ輸送する．ここに，**粘性**は分子粘性ではなく，磁気回転不安定による乱流の有効粘性である．ガスは粘性摩擦によって加熱される．
>磁気回転不安定性：差動回転する磁気流体に普遍的に起こる不安定性のことで，回転軸に対して微小変動を受けた内側と外側のプラズマの角運動量が，プラズマを貫く磁力線を介して交換されることにより成長する。

>差動回転：（流体）連続体が回転運動する場合，各部の角速度が異なるであれば差動回転という．

光蒸発：中心星のX線・紫外線によってガスが加熱され，円盤が蒸発すること．光蒸発は円盤の寿命を短くする．

$$
\frac{\partial\Sigma_\mathrm{g}}{\partial t} = \frac{3}{r}\frac{\partial}{\partial r}\left[r^{1/2}\frac{\partial}{\partial r}(\nu r^{1/2}\Sigma_\mathrm{g})\right] + \frac{1}{r}\frac{\partial}{\partial r}(r\Sigma_\mathrm{g} v_\mathrm{DW}) + \dot{\Sigma}_{\mathrm{g,DW}} + \dot{\Sigma}_{\mathrm{g,PE}}
$$
左辺$\frac{\partial\Sigma_\mathrm{g}}{\partial t}$は円盤の（面）密度が時間変化を表す．右辺第1項
$$
\frac{3}{r}\frac{\partial}{\partial r}\left[r^{1/2}\frac{\partial}{\partial r}(\nu r^{1/2}\Sigma_\mathrm{g})\right] \, \quad 
\left( \Sigma_\mathrm{g} \simeq 1.7\times 10^3\left(\frac{r}{1\,\mathrm{au}}\right)^{-3/2}\,\mathrm{g\,cm^{-2}} \right)
$$
$\nu r^{1/2}\Sigma_\mathrm{g}$，$\nu$が動粘性係数であり，粘性係数との関係は$\nu=\frac{\mu}{\rho}$で，粘性係数を表す．$\nu r^{1/2}\Sigma_\mathrm{g}$を半径方向勾配を取ると，流体粘性による摩擦力．
---

粘性係数：面積$S$，間隔$h$をした2枚平板間に流体をはさみ，平板を相対速度$U$で平行に動かすと，動いている方向に剪断応力$\tau$が発生する．物体と板の間に発生する力$F$を置くと，
$$
\tau = \frac FS = \mu \frac Uh
$$
第1項全体は粘性による角運動量輸送の結果として，ガス面密度$}\Sigma_\mathrm{g}$が時間変化する効果，**粘性拡散項**である．

第2項
$$
\frac{1}{r}\frac{\partial}{\partial r}(r\Sigma_\mathrm{g} v_\mathrm{DW})
$$
$v_\mathrm{DW}$は磁気円盤風によって駆動される降着の速度であり，**磁気円盤風が駆動する半径方向の移流・降着**を表す．

$\dot{\Sigma}_{\mathrm{g,DW}}$は磁気円盤風，$\dot{\Sigma}_{\mathrm{g,PE}}$は光蒸発を表す．2つとも"質量放出率"である．

$\alpha$モデル：$\nu = \alpha c_\mathrm{s} H$を使う．$\alpha = 10^{-4}\text{--}10^{-2}$無次元量．$c_\mathrm{s}$は音速，単位は$[m\,s^{-1}]$；$H$はスケールハイト単位は$[m]$である．右辺の次元は$[m^2\,s^{-1}]$であり，動粘性係数と同じ次元．
